...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    dataset = LocalTextCategorizationDataset(dataset_path, train_conf["batch_size"], 0.8, train_conf["min_samples_per_label"], preprocess_text=embed)
    ...
    model = Sequential([
    #(dataset.get_train_batch()[0].shape[-1],)
    ...
    train_history = model.fit(dataset.get_train_sequence(), epochs=train_conf["epochs"], verbose=train_conf["verbose"], validation_data=dataset.get_test_sequence())
    ...
    with open(artefacts_path+'/labels_index.json', 'w') as outfile:
        ...
        json.dump(labels_index, outfile)
