...
class BaseTextCategorizationDataset:
    ...
    def get_index_to_label_map(self):
        ...
        map = {i: label for i, label in enumerate(labels)}
        return map
    ...
    def get_label_to_index_map(self):
        ...
        labels = self._get_label_list()
        map = {i: label for i, label in enumerate(labels)}
        map_inv = {i: k for k, i in map.items()}
        return map_inv
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        columns_expected = ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']
        assert data.columns.tolist() == columns_expected
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                d_f = df[df['tag_name'].map(df['tag_name'].value_counts()) >= x]
                return d_f

            return filter_function

        pre_processed_dataset = data.pipe(filter_tag_position(0)). \
    ...
    def _get_label_list(self):
        """
        returns label list
        ancien code: list(self._dataset["tag_name"].unique())
        """
